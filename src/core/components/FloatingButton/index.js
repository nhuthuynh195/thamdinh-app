...
const ACTION_BUTTON_SIZE = scaleSzie(50);
...
class FloatingAction extends Component {
  ...
  getIcon = () => {
    ...
    return <AddIcon width={iconWidth} renderIconDone={this.state.done} />;
  };
  ...
  renderMainButton() {
    ...
    if (["left", "right"].indexOf(position) > -1) {
      propStyles[position] = distanceToEdge - scaleSzie(45);
    }

    return (
      <Animated.View
        style={[styles.buttonContainer, propStyles, animatedVisibleView]}
        accessible={true}
        ...
        {sortedActions.map((action) => {
          ...
          return (
            <FloatingActionItem
              ...
              distanceToEdge={distanceToEdge - scaleSzie(45)}
              key={action.name}
              textColor={textColor}
...
FloatingAction.defaultProps = {
  ...
  position: "right",
  distanceToEdge: scaleSzie(60),
  openOnMount: false,
  showBackground: true,
  iconHeight: 15,
  iconWidth: scaleSzie(24),
