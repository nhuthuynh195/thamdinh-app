...
import { Text, Button, ModalCustom, ButtonSubmit,Loading } from '@core/components';
...
export default class PopupEditReviewBusiness extends React.Component {
    ...
    saveChangeInfoBusiness = () => {
        ...
        if (countRow === '' || countRow === '0' || unitPriceRow === '' || unitPriceRow === '0') {
            this.setState({
                errorMessage: 'Vui lòng điền đầy đủ thông tin !'
            })
        } else {
            const table = {
                so_luong: this.splitDotNumber(countRow),
                don_gia: this.splitDotNumber(unitPriceRow),
                thanh_tien_ngay: this.splitDotNumber(dayIntoMoneyRow),
                thanh_tien_thang: this.splitDotNumber(monthIntoMoneyRow),
